...
const Popover = <T extends ValidComponent = "div">(ownProps: PopoverProps<T>) => {
	const [popover, setPopover] = createSignal<HTMLElement | null>(null);
	const [shownState, setShownState] = createSignal<"closed" | "open" | "hiding">("closed");
	...
	const [local, props] = splitProps(
		...
		["id", "ref", "heading", "class", "style", "as", "role", "placement", "children"],
	);
	...
	const resolved = createMemo(() => {
		...
		return typeof child === "function" ? child(() => shownState() !== "closed") : child;
	});
	...
	return (
		...
		>
			<Show when={heading()}>
				<Text with="headline-3" as="header" id={`${local.id}-heading`} class="ms-3 mb-4">
					{heading()}
				</Text>
			</Show>
			{resolved()}
