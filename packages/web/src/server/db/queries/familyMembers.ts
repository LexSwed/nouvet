...
export async function familyMembers(userId: UserID) {
	...
	const users = await db
		...
		.where(
			or(
				and(not(eq(familyUserTable.userId, userId)), eq(familyUserTable.familyId, family)),
				and(eq(familyTable.ownerId, userId), eq(familyWaitListTable.familyId, family)),
			),
		)
...
export async function familyMember(requesterId: UserID, memberId: UserID) {
	...
	const member = await db
		...
		.from(userTable)
		.where(and(eq(familyUserTable.userId, memberId), eq(familyUserTable.familyId, family)))
		.leftJoin(familyUserTable, eq(familyUserTable.userId, userTable.id))
		.leftJoin(familyTable, eq(familyTable.id, family))
